...
export class Temp implements OnInit, ComponentCanDeactivate, AfterViewChecked {
  ...
  constructor(private mainpageservice: MainPageService,
    ...
    events.subscribe('ValidationStatus', (status) => {
      debugger;
      if (status && status != null && status == "Validation has been completed.") {
  ...
  public onSubmoduleSubmit(form: any, dt, ID: any, LinkedMenu: any, moduleIndex: number): void {
    ...
    this.routerEventSubscription = this.router.events.pipe(filter((event): event is NavigationEnd => event instanceof NavigationEnd)
    ).subscribe(() => {//refresh module on reload/navigation like back or submenu click
    ...
    this.mainpageservice.SaveSubmodules(ID, form, dt, this.UserName, appId).subscribe(resp => {
      ...
      this.messageService.add({
        ...
        summary: resp["Message"] == "Record saved successfully!!" ? 'Success' : 'Info',
        detail: resp["Message"],
        life: 8000
      });
      ...
      if (c[prop] != null && ModuleDetail != undefined) {
        if (ModuleDetail.InputControls == "DropDownList" || ModuleDetail.InputControls == "DependentDropdownList") {
          ...
          if (typeof c[prop] == "string") {
            ...
            if (typeof c[prop] === 'string' && c[prop].indexOf(',') > -1) {
              let selected: string[] = c[prop].split(",");
              row[prop] = [];
              selected.forEach(value => {
                const found = ModuleDetail.DropDownValues.find(v => v.Text == value);
                if (found) {
                  row[prop].push(found);
                }
              });
  ...
  UpdateSubmodule(form: any, dt, ID, ButtonId, moduleIndex) {
    ...
    this.mainpageservice.UpdateSubmodules(ID, form, dt, ButtonId, this.UserName).subscribe(resp => {
      ...
      this.messageService.add({
        severity: 'success', // or 'error'/'info' based on resp status
        summary: 'Update Status',
        detail: resp["Message"],
        life: 8000 // duration in ms (same as 8000ms in Ionic)
      });
  ...
  UpdateData(form: any, ID: any, ButtonId: any, LinkedMenu: any) {
    ...
    this.mainpageservice.UpdateData(ID, ButtonId, form, this.UserName).subscribe(resp => {
      // Show PrimeNG toast message
      this.messageService.add({
        severity: resp["Message"]?.toLowerCase().includes('success') ? 'success' : 'info', // Check if message contains "success"
        summary: 'Update Status',
        detail: resp["Message"],
        life: 8000 // Same duration as before (8 seconds)
      });
  ...
  UpdateDatatable(ID: any, ButtonId: any, index: number, dt, filter: any, LinkedMenu: any) {
    ...
    this.mainpageservice.UpdateDatatablewithfilter(ID, this.Sys_Menu_ID, ButtonId, data, this.UserName).subscribe(resp => {
      ...
      const severity = resp["Message"]?.toLowerCase().includes('success') ? 'success' : 'info';
  ...
  SubmitPrimeNgTable(ID: any, filter: any, dt: any, index: number, dtIndex?: number) {
    ...
    this.mainpageservice.SavePrimeNgTable1(ID, this.Sys_Menu_ID, data, this.UserName, appId).subscribe({
      next: (resp) => {
        const isSuccess = resp["Message"] === "Record saved successfully!!";

        // Show PrimeNG toast
        this.messageService.add({
          severity: isSuccess ? 'success' : 'info',
          summary: isSuccess ? 'Success' : 'Information',
          detail: resp["Message"],
          life: 8000,
  ...
  async saveForm(ID, moduleIndex, form, LinkedMenu, appId, filter1) {
    ...
    this.messageService.add({
      severity: 'info', // You can change to 'success', 'error', etc.
      summary: 'Info',
      detail: resp["Message"],
      life: 8000
    });
  ...
  onSubmitFeedback(form: any): void {
    ...
    this.mainpageservice.SaveFeedback(this.Sys_Menu_ID, form, this.UserName).subscribe(resp => {
      ...
      this.messageService.add({
        severity: 'info', // or 'success' if appropriate
        summary: 'Feedback',
        detail: resp["Message"],
        life: 8000
      });
  ...
  Savedatatable(form: any, ID: any): void {
    ...
    this.mainpageservice.SaveDatatable(ID, form, this.UserName, appId).subscribe(resp => {
      ...
      this.messageService.add({
        severity: 'info', // or 'success', 'warn', 'error'
        summary: 'Info',
        detail: resp["Message"],
        life: 8000
      });
  ...
  onWizardSubmit(ID: any): void {
    ...
    this.mainpageservice.Savedata(ID, form, null).subscribe(resp => {
      ...
      this.messageService.add({
        severity: 'info', // you can change this to 'success', 'warn', or 'error' based on your use case
        summary: 'Info',
        detail: resp["Message"],
        life: 8000
      });
  ...
  async Run(i) {
    ...
    this.mainpageservice.RunWorkflowTask(i, this.UserName, undefined).subscribe(async resp => {
      ...
      this.messageService.add({
        severity: 'info', // or 'success', based on context
        summary: 'Info',
        detail: resp["Message"],
        life: 8000
      });
  ...
  SubmitTreeTable(ID: any) {
    ...
    this.mainpageservice.SaveTreetable(ID, dataObject, this.UserName, treetableModuleIndex, appId).subscribe(resp => {
      ...
      this.messageService.add({
        severity: 'info', // Or 'success' if it fits better
        summary: 'Info',
        detail: resp["Message"],
        life: 8000
      });
      ...
      if (this.Module[0].moduleList[moduleIndex].moduleData) {
        ...
        if (Object.keys(event.filters).length > 0)//when column filters applied
        {
          if (!event.filters.hasOwnProperty("global")) {//if filter doesn't have global filters
            ...
            this.totalRecords[dtIndex] = dataToFilter.length;
            console.log(this.totalRecords[dtIndex], 'Paginator');
  ...
  getListView(val, moduleIndex, filterData, appId) {
    this.mainpageservice.populateModuleData(val.ID, this.Sys_Menu_ID, this.UserName, filterData, val.StoredProc, appId).subscribe(data => {
      ...
      this.totalRecords[moduleIndex] = this.listviewdata.length / 9;
      for (let i = 0; i < this.row * 9 && i < this.listviewdata.length; i++) {
    ...
    try {
      ...
      if (this.Module[0].menu.DisplayType.toLowerCase() != "dataview") {
        ...
        moduleList.forEach(val => {
          ...
          else if (val.DisplayType == 'Form' || val.DisplayType.toLowerCase() == "workflow") {//form display 
