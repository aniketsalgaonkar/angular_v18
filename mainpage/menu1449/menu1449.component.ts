...
@Component({
  selector: 'app-menu1449',
  templateUrl: './menu1449.component.html',
  styleUrl: './menu1449.component.scss',
  encapsulation: ViewEncapsulation.None
})
export class Menu1449Component {
  ...
  showEditConstraintTable = false;
  selectedConstraint: string | null = null;
  loadingConstraintData = true;
  scenarioDialogSteps = [
    {
      label: "Scenario",
    },
  ...
  DeleteByID(ID: number, TableName: string) {
    this.mainpageservice.deleteRecord(ID, TableName, this.UserName).subscribe(
      (response) => {
  ...
  disableMultiSelection(
    formObject,
    checkboxName //disable multi selection of checkbox in card view display type
  ) {
  ...
  onChipExpand(row: any) {
    row["ExpandConstraints"] = !row["ExpandConstraints"];
  }
  paginate(event) {
    //debugger;
    ...
    //console.log(event);
    this.paginateData = [];
    this.paginateData = <any[]>(
      this.cardViewData.slice(event.first, event.first + event.rows)
    );
    //console.log(this.paginateData);
  }
  // disableConstraintParameterChip = (value) => {
  //   return true;
  // };
  disableConstraintParameterChip: boolean = true; // Now a boolean, not a function
  openChipSideBar(value: any) {
    this.showEditConstraintTable = true;
    this.selectedConstraint = value.Text;
    this.getConstraintTableData(value.ModuleId).then(
      () => (this.loadingConstraintData = false)
    );
  }
  async getConstraintTableData(moduleId: number) {
    this.loadingConstraintData = true;
    const module: any = await this.mainpageservice
      .getModuleWithId(moduleId, this.UserName)
      .toPromise();
    const { moduleDetails } = module;
    this.constraintTableData = module;
    this.constraintTableColumns = moduleDetails;
  }
  onAddConstraintDataClick(data: any, index: any) {
    let row = this.constraintTableColumns.reduce((acc, curr) => {
      acc[curr.ColumnName] = null;
      return acc;
    }, {});
    ...
    if (!!data) {
      this.newrow = false;
      row = Object.entries(data).reduce((acc, [key, value]) => {
        const md = this.constraintTableColumns.find(
          (md) => md.ColumnName == key
        );
        const inputControl = md?.InputControls.toLowerCase();
        if (["dropdownlist", "dependentdropdownlist"].includes(inputControl)) {
          acc[key] = md?.DropDownValues?.find((dv) => dv.Text == value);
        } else if (["multidropdownlist"].includes(inputControl)) {
          const values = (value as string).split(",");
          acc[key] = md?.DropDownValues?.filter((dv) =>
            values.includes(dv.Text)
          );
        } else {
          acc[key] = value;
        }
    ...
    if (index != undefined) this.selectedCellValue = index;

    const newId = Math.random().toString(36).substr(2, 9);
    row["ID"] = this.newrow ? `new-${newId}` : row["ID"];

    this.constraintDataDialog = {
      show: true,
      title: this.selectedConstraint,
      row,
    };
  }
  clear(table: Table) {
  ...
  clearPassword(col: any) {
    col.value = ''; // Clear the password value
    // If you need to trigger filtering after clearing
    if (col.ColumnSearchEnable !== false) {
      this.dt.filter('', col.ColumnName, 'contains');
    }
  }
  decryptPassword(event: any, password: string) {
  ...
  closeChipSidebar() {
    if (this.showEditConstraintTable) this.showEditConstraintTable = false;
    else if (this.activeStepIndex > 0) this.activeStepIndex--;
    else {
      this.addNew = false;
    }
    this.loadingConstraintData = true;
    this.showEditConstraintTable = false;
    this.selectedConstraint = null;
    this.constraintTableData = [];
    this.constraintTableColumns = [];
  }
  onNextClick(formData: any, menuId: number) {
    if (this.scenarioCloned) {
      console.log(this.scenarioCloned);
      console.log(this.paginateData);
      this.scenarioCloned = null;
      this.addNew = false;
    }
    if (this.activeStepIndex < this.scenarioDialogSteps.length - 1)
      this.activeStepIndex++;
    else if (this.showEditConstraintTable) this.saveTable();
    else {
      if (!formData.form.valid) return;
      if (this.dialogLabel != "Edit") {
        this.paginateData = [
          ...this.paginateData,
          {
            ...this.thisrow,
            ID: null,
            Objective: this.thisrow?.Objective?.Text,
            Constraints: this.thisrow?.Constraints.filter((c) => !!c).map(
              (c) => c.Text
            ),
          },
        ];
      }
      formData.form.value = { ...this.thisrow };
      this.onSubmit(formData, menuId, 0, 0);
      this.addNew = false;
    }
  }
  saveTable() {
    this.show = true;
    this.constraintTableData.moduleData =
      this.constraintTableData.moduleData.map((row) => {
        row.ID = typeof (row.ID) === "string" && row.ID?.includes("new-") ? null : row.ID;
        return row;
      });
    this.mainpageservice
      .SavePrimeNgTable1(
        this.constraintTableData.ID,
        this.Sys_Menu_ID,
        { data: this.constraintTableData.moduleData },
        this.UserName,
        this.CurrentApp.ID
      )
      .subscribe((resp) => {
        ...
        this.messageService.add({
          ...
          summary: 'Success', // Optional header
          detail: resp["Message"], // Your API response message
          life: 8000 // Duration in ms (matches your toast duration)
        });

        this.show = false;
        this.closeChipSidebar();
      });
  }
  ...
  navigateOnFormSubmit(LinkedMenu) {
    ...
    this.router.navigateByUrl(
      "/menu/first/tabs/" + LinkedMenu,
      navigationExtras
    );
  }

  getNextButtonLabel() {
    if (this.showEditConstraintTable) return "Update";
    else if (this.activeStepIndex == this.scenarioDialogSteps.length - 1) {
      if (this.dialogLabel === "Edit") return "Update";
      else return "Save";
    } else return "Next";
  }
  ...
  onCloneClick() {
    ...
    this.paginateData = [...this.paginateData, this.scenarioCloned];
    this.thisrow = this.scenarioCloned;
    this.activeStepIndex = 0;
    // this.addNew = false;
  }


  GetValuesForDependentDropdown(
    DDModuleDetailID,
    filterByValue,
    moduleIndex,
    subModuleIndex?: number,
    fetchForConstraintData = false
  ) {
    if (subModuleIndex != null && subModuleIndex != undefined) {
      var dd = this.Module[0].moduleList[moduleIndex].submodule[
        subModuleIndex
      ].subModuleDetails.filter((md) => md.ID == DDModuleDetailID);
      //console.log(dd);
      this.mainpageservice
        .MultipleDependentDropdown(
          DDModuleDetailID,
          filterByValue.Text,
          dd[0].DropDownSP,
          this.UserName
        ...
        .subscribe((resp) => {
          ...
          dd.forEach((md) => {
            var index = "";
            var dependentDropdown = this.Module[0].moduleList[
              moduleIndex
            ].submodule[subModuleIndex].subModuleDetails.find(
              (smd) => smd.ID == md.ID
            );
            if (i != 0) index = "" + i;
    ...
    if (fetchForConstraintData) {
      const md = this.constraintTableColumns.find(
        (md) => md.ID == DDModuleDetailID
      );
      if (md) {
        const ddMd = this.constraintTableColumns.find(
          (m) => m.ID == md.DependentDDID
        );
        ddMd.Loading = true;
        if (Array.isArray(filterByValue)) {
          ...
          this.mainpageservice
            ...
            .subscribe((resp: any) => {
              if (ddMd) {
                ddMd.DropDownValues = resp.Table;
                ddMd.Loading = false;
              }
            });
        } else {
  ...
  saveRecordInCrudTable(
    module_dt_count: number,
    tableData: Array<any> = undefined
  ) {
    //Temporary save of record in crudtable for browser app only// data is not added to database until submit button is clicked
    let thisrows = [];
    var row = this.thisrow;
    if (!!tableData) {
      thisrows = tableData;
      row = this.constraintDataDialog.row;
    } else {
      thisrows = [...this.primeNgTableArray[module_dt_count]];
    }
    Object.keys(row).map(function (key, index) {
      if (row[key] != null) {
        if (row[key].hasOwnProperty("Text")) row[key] = row[key].Text;
        ...
        if (
          ...
          multiDropdownValues = multiDropdownValues.substring(
            0,
            multiDropdownValues.length - 1
          );
    ...
    if (!!tableData) this.constraintDataDialog.row = row;
    else this.thisrow = row;
    //console.log(this.thisrow, 'rowdata');
    if (this.newrow) {
      thisrows.push(!!tableData ? this.constraintDataDialog.row : this.thisrow);
      ////console.log(this.thisrow,'newrowdata');
    } else {
      if (!!tableData) {
        thisrows[this.selectedCellValue] = this.constraintDataDialog.row;
      } else {
        thisrows[
          this.primeNgTableArray[module_dt_count].indexOf(this.selectedValues)
        ] = this.thisrow;
      }
    ...
    if (!!tableData) {
      tableData = thisrows;
      this.constraintDataDialog.row = null;
      this.constraintDataDialog.show = false;
    } else {
  ...
  onResetConstraintDataClick() {
    this.constraintDataDialog.row = Object.keys(
      this.constraintDataDialog.row
    ).reduce((acc, key) => {
      acc[key] = null;
      return acc;
    }, {});
  ...
  onDeleteConstraintDataClick() {
    if (
      typeof (this.constraintDataDialog.row["ID"]) === "string" &&
      this.constraintDataDialog.row["ID"].includes("new-")
    )
      this.constraintTableData.moduleData =
        this.constraintTableData.moduleData.filter(
          (current) => current.ID !== this.constraintDataDialog.row["ID"]
        );
    else
      this.DeleteByID(
        this.constraintDataDialog.row["ID"],
        this.constraintTableColumns[0].TableName
      );
    ...
    this.getConstraintTableData(this.constraintTableData.ID).then(() => {
      this.loadingConstraintData = false;
    })
  }

  async NavigateRunScenario(id: number) {
    this.router.navigateByUrl(`/menu/first/tabs/${id}`)
  }
