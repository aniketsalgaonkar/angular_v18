...
@Component({
  selector: 'app-menu129',
  templateUrl: './menu129.component.html',
  styleUrl: './menu129.component.scss',
  encapsulation: ViewEncapsulation.None

})
export class Menu129Component {
  ...
  SubmitPrimeNgTable(ID: any, filter: any, dt: any, index: number, dtIndex?: number) {
    ...
    this.menuService.SavePrimeNgTable1(ID, this.Sys_Menu_ID, data, this.UserName, appId).subscribe({
      next: (resp: any) => {
      ...
      error: (err: any) => {
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: err.message || 'An error occurred while saving the record.',
          life: 8000
        });
  ...
  loadTableData(event: any, dtIndex: any, moduleIndex: any) {
    ...
    setTimeout(() => {
      ...
      console.log("primeNgTableArray", that.primeNgTableArray);
  ...
  getTableModule(val: any, moduleIndex: any, tableModuleList: any, filterData: any, appId: any) {
    ...
    if (val.moduleDetails != null && typeof (val.moduleDetails) != undefined) {
      ...
      if (this.scrollableColsArray[0]) {
        this.globalFilterFields272 = this.scrollableColsArray[0].map(col => col.ColumnName);
      }
