...
@Component({
  selector: 'app-form-table-submodule',
  templateUrl: './form-table-submodule.component.html',
  styleUrl: './form-table-submodule.component.scss'
})
export class FormTableSubmoduleComponent implements OnInit {
  selectedFile: File[][] = [];
  dialogLabel: string = '';
  tableName: any;
  RowErrors: any;
  ...
  //form table submodule
  onBasicUploadAuto(event: any, index: number) {
    debugger;
    const files = event.target.files;
    if (files && files.length > 0) {
      // this.selectedFile[index] = files; 
      this.selectedFile[index] = Array.from(files);
      console.log(this.selectedFile[index]);
      // Extract file names and concatenate them into a comma-separated string
      const commaSeparatedFileNames = this.selectedFile[index].map(file => file.name).join(',');
      this.fileNames[index] = commaSeparatedFileNames;
      console.log("filename", this.fileNames[index]);
    }
  }


  public UploadtoDynamicFolder(form: any, dt, ID: any, LinkedMenu: any, moduleIndex: number): void {
    ...
    this.menuService.UploadtoDynamicFolder(ID, form, dt, this.selectedFile, this.UserName, appId).subscribe(resp => {
      ...
      if (resp["Message"] == "Record saved successfully!!") {
        ...
        if (resp["Message"].includes("Record saved successfully!!")) {
          this.messageService.add({ 
            severity: 'success', 
            summary: 'TPO Created Successfully', 
            detail: '', 
            life: 3000 
          });
  ...
  async openTable(ModuleId: any, product: any, item: any) {
    console.log("item", item);
    console.log("product", product);
    console.log("ModuleId", ModuleId);
    this.tableName = product;
    //opens dialog to validate table module
    let module = this.Module[0].moduleList.find((m: any) => m.ID == ModuleId);



    if (!Array.isArray(item.RowError) && item.RowError && item.RowError !== "undefined") {
      this.RowErrors = JSON.parse(item.RowError);

    }
    console.log("this.RowErrors", this.RowErrors);
    ...
    let appId: any = null;
    this.CurrentApp = JSON.parse(localStorage.getItem("currentApp") || 'null');
    if (this.CurrentApp != null || this.CurrentApp != undefined) appId = this.CurrentApp.ID;
  ...
  finalSubmitConfirmation(i: any, Event: any) {
  ...
  async Run(i: any) {
    let qm = { "rowval": i };
    this.UserName = localStorage.getItem('username') || ''; // Add fallback for null
    this.show = true;
    ...
    this.messageService.add({
      severity: 'info',
      summary: 'Processing',
      detail: 'Running workflow task...',
      life: 3000
    });

    this.menuService.RunWorkflowTask(i, this.UserName, undefined).subscribe({
      next: async (resp: any) => {
        // Success notification
        this.messageService.add({
          severity: 'success',
          summary: 'Success',
          detail: resp.Message || 'Task completed successfully', // Safely access Message property
          life: 8000
        });
      ...
      error: (err: any) => {
        ...
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: err.message || 'Failed to run workflow task',
          life: 8000
        });
        this.show = false;
      }
  ...
  async saveForm(ID, moduleIndex, form, LinkedMenu, appId, filter1) {
    ...
    let formObject = form;
    const resp = await this.menuService.Savedata(ID, form, this.UserName, appId, filter1).toPromise();
