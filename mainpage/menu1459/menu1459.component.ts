import { Component, OnInit, ViewChild, ViewChildren, AfterViewChecked, QueryList, ElementRef, HostListener, ViewEncapsulation, OnDestroy } from '@angular/core';
...
@Component({
  selector: 'app-menu1459',
  templateUrl: './menu1459.component.html',
  styleUrl: './menu1459.component.scss',
  encapsulation: ViewEncapsulation.None
})


export class Menu1459Component implements OnInit, OnDestroy, ComponentCanDeactivate, AfterViewChecked {
    ...
    try {
      ...
      if (this.Module[0].menu.DisplayType.toLowerCase() != "dataview") {
        ...
        moduleList.forEach((val: any) => {
          ...
          if (val.DisplayType.toLowerCase() == "processflow") {
  ...
  getTaskRows() {
    if (!this.Module?.[0]?.moduleList?.[0]?.moduleData) return [];
    const maxSubtasks = Math.max(...this.Module[0].moduleList[0].moduleData.map(p => p.SubTasks?.length || 0));
    const rows = [];
    this.show = false;
    for (let i = 0; i < maxSubtasks; i++) {
      const row = {};
      this.Module[0].moduleList[0].moduleData.forEach(process => {
        row[process.Name] = process.SubTasks?.[i];
      });
      rows.push(row);
    }
    return rows;
